      ...
      format_string )
{
	...
	if( error != NULL )
	{
		if( libcerror_error_backtrace_sprint(
		     error,
		     error_string,
		     PYFWSI_ERROR_STRING_SIZE ) != -1 )
		{
			while( error_string_index < PYFWSI_ERROR_STRING_SIZE )
			{
			...
			if( error_string_index >= PYFWSI_ERROR_STRING_SIZE )
			{
				error_string[ PYFWSI_ERROR_STRING_SIZE - 1 ] = 0;
			}
			PyErr_Format(
			 exception_object,
			 "%s %s",
