...
LIBRARY_TESTS="error storage support value";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./fwps_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfwps" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
